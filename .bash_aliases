...
kseal() {
  ...
  echo -n $2 | kubectl create secret generic $1 --dry-run=client --from-file=key=/dev/stdin -o yaml | kubeseal --controller-name=sealed-secrets --controller-namespace=argocd --scope=cluster-wide -o yaml -w sealedsecret-$1.yaml
